...
-- // Variables
local players, http, runservice, inputservice, tweenService, stats, actionservice =
    gs('Players'),
    gs('HttpService'),
    gs('RunService'),
    gs('UserInputService'),
    gs('TweenService'),
    gs('Stats'),
    gs('ContextActionService')
local localplayer = players.LocalPlayer

local setByConfig = false
local floor, ceil, huge, pi, clamp =
    math.floor, math.ceil, math.huge, math.pi, math.clamp
local c3new, fromrgb, fromhsv = Color3.new, Color3.fromRGB, Color3.fromHSV
local next, newInstance, newUDim2, newVector2 =
    next, Instance.new, UDim2.new, Vector2.new
...
local library = {
    ...
    signal = loadstring(
        game:HttpGet(
            'https://raw.githubusercontent.com/drillygzzly/Other/main/1414'
        )
    )(),
    open = false,
    opening = false,
...
local button1down, button1up, mousemove =
    library.button1down, library.button1up, library.mousemove
local mb1down = false
...
do
    ...
    function utility:ConvertNumberRange(val, oldmin, oldmax, newmin, newmax)
        return (((val - oldmin) * (newmax - newmin)) / (oldmax - oldmin))
            + newmin
    end
    ...
    function utility:UDim2ToVector2(udim2, vector2)
        local x, y
        x = udim2.X.Offset
            + self:ConvertNumberRange(udim2.X.Scale, 0, 1, 0, vector2.X)
        y = udim2.Y.Offset
            + self:ConvertNumberRange(udim2.Y.Scale, 0, 1, 0, vector2.Y)
    ...
    function utility:Tween(obj, prop, val, time, direction, style)
        if self:HasProperty(obj, prop) then
            ...
            tween.Connection = self:Connection(
                ...
                function(dt)
                    ...
                    pcall(function()
                        local progress = tweenService:GetValue(
                            a,
            ...
            printconsole(
                'unable to tween: invalid property '
                    .. tostring(prop)
                    .. ' for object '
                    .. tostring(obj),
                255,
    ...
    function utility:DetectTableChange(indexcallback, newindexcallback)
        if indexcallback == nil then
            warn(
                'DetectTableChange: Argument #1 (indexcallback) is nil, function may not work as expected.'
            )
        elseif newindexcallback == nil then
            warn(
                'DetectTableChange: Argument #2 (newindexcallback) is nil, function may not work as expected.'
            )
        end
    ...
    function utility:Draw(class, properties)
        ...
        function drawing:Update()
            ...
            local parent = drawing.Parent ~= nil
                    and library.drawings[drawing.Parent.Object]
                or nil
            local parentSize, parentPos, parentVis =
                workspace.CurrentCamera.ViewportSize, Vector2.new(0, 0), true
            if parent ~= nil then
                parentSize = (
                    parent.Class == 'Square' or parent.Class == 'Image'
                )
                        and parent.Object.Size
                    or parent.Class == 'Text' and parent.TextBounds
            ...
            if drawing.Class == 'Square' or drawing.Class == 'Image' then
                drawing.Object.Size = typeof(drawing.Size) == 'Vector2'
                        and drawing.Size
                    or typeof(drawing.Size) == 'UDim2'
                        and utility:UDim2ToVector2(drawing.Size, parentSize)
            end

            if
                drawing.Class == 'Square'
                or drawing.Class == 'Image'
                or drawing.Class == 'Circle'
                or drawing.Class == 'Text'
            then
                drawing.Object.Position = parentPos
                    + (
                        typeof(drawing.Position) == 'Vector2'
                            and drawing.Position
                        or utility:UDim2ToVector2(drawing.Position, parentSize)
                    )
            end

            drawing.Object.Visible = (parentVis and drawing.Visible) and true
                or false
        ...
        local proxy = utility:DetectTableChange(function(obj, i)
            ...
            if not table.find(blacklistedProperties, i) then
                ...
                then
                    drawing.Object.Position = utility:UDim2ToVector2(
                        v,
                        drawing.Parent == nil and newVector2(0, 0)
                            or drawing.Parent.Object.Position
                    )
                ...
                if
                    table.find(
                        { 'Size', 'Position', 'Position', 'Visible', 'Parent' },
                        i
                    )
                then
                    drawing:Update()
...
function library:init()
    ...
    function self:LoadConfig(name)
        ...
        if not cfg then
            self:SendNotification(
                'Error loading config: Config does not exist. ('
                    .. tostring(name)
                    .. ')',
                5,
                c3new(1, 0, 0)
                ...
                if option ~= nil then
                    ...
                    elseif option.class == 'bind' then
                        option:SetBind(
                            value == nil and 'none'
                                or (
                                    utility:HasProperty(Enum.KeyCode, value)
                                        and Enum.KeyCode[value]
                                    or Enum.UserInputType[value]
                                )
                        )
                    elseif option.class == 'color' then
                        option:SetColor(
                            value == nil and c3new(1, 1, 1)
                                or c3new(value[1], value[2], value[3])
                        )
        ...
        if s then
            self:SendNotification(
                'Successfully loaded config: ' .. name,
                5,
                c3new(0, 1, 0)
            )
        else
    ...
    function self:SaveConfig(name)
        if not self:GetConfig(name) then
            self:SendNotification(
                'Error saving config: Config does not exist. ('
                    .. tostring(name)
                    .. ')',
                5,
                c3new(1, 0, 0)
        ...
        if s then
            self:SendNotification(
                'Successfully saved config: ' .. name,
                5,
                c3new(0, 1, 0)
            )
        else
    ...
    self.cursor2 = utility:Draw(
        'Triangle',
        { Filled = true, Color = fromrgb(85, 85, 85), self.zindexOrder.cursor - 1 }
    )
            ...
            if library.open then
                ...
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    ...
                    if hoverObj and hoverObjData then
                        hoverObjData.MouseButton1Down:Fire(
                    ...
                    if library.draggingSlider ~= nil then
                        local rel = inputservice:GetMouseLocation()
                            - library.draggingSlider.objects.background.Object.Position
                        local val = utility:ConvertNumberRange(
                            rel.X,
                            0,
                            library.draggingSlider.objects.background.Object.Size.X,
                            library.draggingSlider.min,
                            library.draggingSlider.max
                        )
                ...
                then
                    if hoverObj and hoverObjData then
                        hoverObjData.MouseButton2Down:Fire(
                            inputservice:GetMouseLocation()
                        )
                    end
                end
    ...
    utility:Connection(inputservice.InputEnded, function(input, gpe)
        if self.hasInit and library.open then
            ...
            if input.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                button1up:Fire()
                if hoverObj and hoverObjData then
                    hoverObjData.MouseButton1Up:Fire(
                        inputservice:GetMouseLocation()
                    )
                end
            elseif input.UserInputType == Enum.UserInputType.MouseButton2 then
                if hoverObj and hoverObjData then
                    hoverObjData.MouseButton2Up:Fire(
                        inputservice:GetMouseLocation()
                    )
                end
            end
        ...
        if input.UserInputType == Enum.UserInputType.MouseMovement then
            if library.open then
                ...
                if library.CurrentTooltip ~= nil then
                    local mousePos = inputservice:GetMouseLocation()
                    tooltipObjects.background.Position =
                        UDim2.new(0, mousePos.X + 15, 0, mousePos.Y + 15)
                    tooltipObjects.background.Size = UDim2.new(
                        0,
                        tooltipObjects.text.TextBounds.X
                            + 6
                            + (library.CurrentTooltip.risky and 60 or 0),
                        0,
                        tooltipObjects.text.TextBounds.Y + 2
                    )
                end
    ...
    function self.UpdateThemeColors()
        for _, v in next, library.drawings do
            if v.ThemeColor and library.theme[v.ThemeColor] then
                v.Object.Color = utility:AddRGB(
                    library.theme[v.ThemeColor],
                    fromrgb(
                        v.ThemeColorOffset,
                        v.ThemeColorOffset,
                        v.ThemeColorOffset
            ...
            if v.ThemeColorOutline and library.theme[v.ThemeColorOutline] then
                v.Object.OutlineColor = utility:AddRGB(
                    library.theme[v.ThemeColorOutline],
                    fromrgb(
                        v.OutlineThemeColorOffset,
                        v.OutlineThemeColorOffset,
                        v.OutlineThemeColorOffset
                    )
                )
    ...
    function self:SendNotification(message, time, color)
        ...
        if typeof(message) ~= 'string' then
            return error(
                string.format(
                    'invalid message type, got %s, expected string',
                    typeof(message)
                )
            )
        elseif typeof(time) ~= 'number' then
            return error(
                string.format(
                    'invalid time type, got %s, expected number',
                    typeof(time)
                )
            )
        elseif color ~= nil and typeof(color) ~= 'Color3' then
            return error(
                string.format(
                    'invalid color type, got %s, expected color3',
                    typeof(time)
                )
            )
        end
        ...
        task.spawn(function()
            ...
            notification.background.Size =
                newUDim2(0, notification.text.TextBounds.X + 20, 0, 19)
            task.wait()
            utility:Tween(
    ...
    function self.NewIndicator(data)
        ...
        function indicator:Update()
            ...
            for _, v in next, self.values do
                ...
                v.objects.valueLabel.Position =
                    newUDim2(1, -(v.objects.valueLabel.TextBounds.X + 3), 0, 0)
                ...
                if v.enabled then
                    yPos = yPos + 16 + 3
                    local x = (
                        v.objects.keyLabel.TextBounds.X
                        + 10
                        + v.objects.valueLabel.TextBounds.X
                    )
                    if x > xSize then
                        xSize = x
                    end
        ...
        function indicator:AddValue(data)
            ...
            function value:Remove()
                table.remove(
                    indicator.values,
                    table.find(indicator.values, value)
                )
    ...
    function self.NewWindow(data)
        ...
        do
            ...
            objs.columnholder1 = utility:Draw('Square', {
                Size = newUDim2(0.48, 0, 0.96, 0),
                Position = newUDim2(0.01, 0, 0.02, 0),
                Transparency = 0,
                ZIndex = z + 6,
                Parent = objs.groupBackground,
            })
            ...
            objs.columnholder2 = utility:Draw('Square', {
                Size = newUDim2(0.48, 0, 0.96, 0),
                Position = newUDim2(1 - (0.48 + 0.01), 0, 0.02, 0),
                Transparency = 0,
                ZIndex = z + 6,
                Parent = objs.groupBackground,
            })
            ...
            utility:Connection(mousemove, function(pos)
                if dragging then
                    if window.open then
                        objs.background.Position = objStart
                            + newUDim2(0, pos.X, 0, pos.Y)
                            - mouseStart
                    else
                        dragging = false
                    end
        ...
        do
            ...
            do
                ...
                local draggingHue, draggingSat, draggingTrans =
                    false, false, false
                ...
                local function updateSatVal(pos)
                    if window.colorpicker.selected ~= nil then
                        ...
                        X = (
                            ...
                            + objs.mainColor.Object.Size.X
                        )
                            - objs.mainColor.Object.Position.X
                        Y = (
                            objs.mainColor.Object.Position.Y
                            + objs.mainColor.Object.Size.Y
                        )
                            - objs.mainColor.Object.Position.Y
                        X = math.clamp(
                            (pos.X - objs.mainColor.Object.Position.X) / X,
                            0,
                            0.995
                        )
                        Y = math.clamp(
                            (pos.Y - objs.mainColor.Object.Position.Y) / Y,
                            0,
                            0.995
                        )
                        sat, val = 1 - X, 1 - Y
                        window.colorpicker.selected:SetColor(
                            fromhsv(hue, sat, val)
                        )
                        window.colorpicker:Visualize(
                            fromhsv(hue, sat, val),
                            window.colorpicker.selected.trans
                        )
                    end
                end
                ...
                local function updateHue(pos)
                    if window.colorpicker.selected ~= nil then
                        ...
                        X = math.clamp(
                            (pos.X - objs.hue.Object.Position.X) / X,
                            0,
                            0.995
                        )
                        hue = 1 - X
                ...
                local function updateTrans(pos)
                    if window.colorpicker.selected ~= nil then
                        Y = (
                            ...
                            + objs.trans.Object.Size.Y
                        )
                            - objs.trans.Object.Position.Y
                        Y = math.clamp(
                            (pos.Y - objs.transColor.Object.Position.Y) / Y,
                            0,
                            0.995
                        )
                ...
                utility:Connection(
                    objs.mainDetector.MouseButton1Down,
                    function(pos)
                        draggingSat = true
                        updateSatVal(pos)
                    end
                ...
                utility:Connection(
                    objs.hueDetector.MouseButton1Down,
                    function(pos)
                        draggingHue = true
                        updateHue(pos)
                    end
                ...
                utility:Connection(
                    objs.transDetector.MouseButton1Down,
                    function(pos)
                        draggingTrans = true
                        updateTrans(pos)
                    end
            ...
            function window.colorpicker:Visualize(c3, a)
                ...
                if self.selected ~= nil then
                    ...
                    elseif
                        self.selected.flag ~= nil
                        and self.selected.flag ~= ''
                    then
            ...
            function window.dropdown:Refresh()
                if self.selected ~= nil then
                            ...
                            valueObject.connection = utility:Connection(
                                ...
                                function()
                                    ...
                                    if currentList then
                                        ...
                                        local newSelected = currentList.multi
                                                and {}
                                            or val
                                        ...
                                        if not currentList.multi then
                                            ...
                                            window.dropdown.objects.background.Visible =
                                                false
                                        end

                                        for idx, val in next, currentList.values do
                                            local valueObj =
                                                self.objects.values[idx]
                                            if valueObj then
                    ...
                    for idx, val in next, list.values do
                        ...
                        if valueObj then
                            valueObj.background.Transparency = (
                                typeof(list.selected) == 'table'
                                    and table.find(list.selected, val)
                                or list.selected == val
                            )
                                    and 1
                                or 0
                        end
        ...
        local function tooltip(option)
            utility:Connection(option.objects.holder.MouseEnter, function()
                tooltipObjects.background.Visible = not (
                    option.tooltip == '' or option.tooltip == nil
                )
                        and true
                    or false
                tooltipObjects.riskytext.Visible = option.risky
                tooltipObjects.text.Position = option.risky
                        and newUDim2(0, 60, 0, 0)
                    or newUDim2(0, 3, 0, 0)
        ...
        function window:AddTab(text, order)
            ...
            function tab:AddSection(text, side, order)
                ...
                do
                    ...
                    objs.textlabel = utility:Draw('Text', {
                        Position = newUDim2(0.0425, 0, 0, -7),
                        ThemeColor = 'Primary Text',
                        Size = 13,
                        Font = 2,
                        ZIndex = z + 1,
                    ...
                    objs.optionholder = utility:Draw('Square', {
                        Size = newUDim2(1 - 0.03, 0, 1, -15),
                        Position = newUDim2(0.015, 0, 0, 13),
                        Transparency = 0,
                        ZIndex = z + 1,
                        Parent = objs.background,
                    })
                ...
                function section:SetText(text)
                    ...
                    local x = self.objects.background.Object.Size.X
                ...
                function section:AddToggle(data)
                    ...
                    do
                        ...
                        utility:Connection(objs.holder.MouseLeave, function()
                            objs.border1.ThemeColor = toggle.state and 'Accent'
                                or 'Option Border 1'
                        end)

                        utility:Connection(
                            objs.holder.MouseButton1Down,
                            function()
                    ...
                    function toggle:SetState(bool, nocallback)
                        if typeof(bool) == 'boolean' then
                            ...
                            self.objects.border1.ThemeColor = bool and 'Accent'
                                or (
                                    self.objects.holder.Hover and 'Accent'
                                    or 'Option Border 1'
                                )
                            self.objects.text.ThemeColor = bool
                                    and (self.risky and 'Risky Text Enabled' or 'Option Text 1')
                                or (
                                    self.risky and 'Risky Text'
                                    or 'Option Text 3'
                                )
                            self.objects.background.ThemeColor = bool
                                    and 'Accent'
                                or 'Option Background'
                            self.objects.background.ThemeColorOffset = bool
                                    and -55
                                or 0
                    ...
                    function toggle:UpdateOptions()
                        ...
                        for i, option in next, self.options do
                            ...
                            if option.enabled then
                                if
                                    option.class == 'color'
                                    or option.class == 'bind'
                                then
                                    option.objects.holder.Position = newUDim2(
                                        1,
                                        -option.objects.holder.Object.Size.X - x,
                                        0,
                                        0
                                    )
                                ...
                                elseif
                                    option.class == 'slider'
                                    or option.class == 'list'
                                then
                                    option.objects.holder.Position = newUDim2(
                                        0,
                                        0,
                                        1,
                                        -option.objects.holder.Object.Size.Y - y
                                    )
                    ...
                    function toggle:AddColor(data)
                        ...
                        do
                            ...
                            utility:Connection(
                                objs.holder.MouseLeave,
                                function()
                                    objs.border1.ThemeColor = color.state
                                            and 'Accent'
                                        or 'Option Border 1'
                                end
                            )
                        ...
                        function color:SetOpen(bool)
                            if typeof(bool) == 'boolean' then
                                ...
                                if bool then
                                    ...
                                    window.colorpicker.selected = color
                                    window.colorpicker.objects.background.Parent =
                                        self.objects.background
                                    window.colorpicker.objects.background.Visible =
                                        true
                                    window.colorpicker:Visualize(
                                        color.color,
                                        color.trans
                    ...
                    function toggle:AddBind(data)
                        local bind = {
                            ...
                            indicatorValue = library.keyIndicator:AddValue({
                                value = 'value',
                                key = 'key',
                                enabled = false,
                            }),
                        ...
                        if bind.bind == 'none' then
                            ...
                            bind.indicatorValue:SetEnabled(
                                display and not bind.noindicator
                            )
                            bind.indicatorValue:SetKey(
                                (bind.text == nil or bind.text == '')
                                        and (bind.flag == nil and 'unknown' or bind.flag)
                                    or bind.text
                            ) -- this is so dumb
                        ...
                        do
                            ...
                            utility:Connection(
                                objs.holder.MouseLeave,
                                function()
                                    objs.keyText.ThemeColor = bind.binding
                                            and 'Accent'
                                        or 'Option Text 3'
                                end
                            )
                        ...
                        function bind:SetBind(keybind)
                            ...
                            self.indicatorValue:SetKey(
                                (self.text == nil or self.text == '')
                                        and (self.flag == nil and 'unknown' or self.flag)
                                    or self.text
                            ) -- this is so dumb
                            ...
                            self.objects.keyText.ThemeColor = self.objects.holder.Hover
                                    and 'Accent'
                                or 'Option Text 3'
                        end
                        ...
                        function bind:SetKeyText(str)
                            ...
                            self.objects.holder.Size = newUDim2(
                                0,
                                self.objects.keyText.TextBounds.X + 2,
                                0,
                                17
                            )
                        ...
                        utility:Connection(
                            ...
                            function(inp)
                                ...
                                elseif bind.binding then
                                    ...
                                    bind.binding = false
                                elseif
                                    not bind.binding and self.bind == 'none'
                                then
                                    bind.state = true
                                    library.flags[bind.flag] = bind.state
                                    local display = bind.state
                                ...
                                elseif
                                    (
                                        inp.KeyCode == bind.bind
                                        or inp.UserInputType == bind.bind
                                    )
                                    and not bind.binding
                                then
                                    if bind.mode == 'toggle' then
                                        bind.state = not bind.state
                                        if bind.flag then
                                            library.flags[bind.flag] =
                                                bind.state
                                        end
                                    ...
                                    elseif bind.mode == 'hold' then
                                        ...
                                        bind.indicatorValue:SetEnabled(
                                            (
                                                not bind.invertindicator
                                                    and true
                                                or false
                                            )
                                                and not bind.noindicator
                                        )
                    ...
                    function toggle:AddSlider(data)
                        ...
                        do
                            ...
                            utility:Connection(
                                objs.holder.MouseLeave,
                                function()
                                    objs.border1.ThemeColor = slider.dragging
                                            and 'Accent'
                                        or 'Option Border 1'
                                end
                            )
                            ...
                            local inputNumber = ''
                            utility:Connection(
                                slider.objects.holder.MouseButton1Down,
                                function()
                                    if
                                    ...
                                    then
                                        if slider.focused then
                                            ...
                                            c = utility:Connection(
                                                ...
                                                function(inp)
                                                    if
                                                        library.numberStrings[inp.KeyCode.Name]
                                                    then
                                                        local number =
                                                            library.numberStrings[inp.KeyCode.Name]
                                                        inputNumber = inputNumber
                                                            .. tostring(number)
                                                        objs.text.Text = string.format(
                                                            '%.14g',
                                                            slider.value
                                                        ) .. tostring(
                                                            slider.suffix
                            ...
                            utility:Connection(button1up, function()
                                objs.border1.ThemeColor = objs.holder.Hover
                                        and 'Accent'
                                    or 'Option Border 1'
                        ...
                        function slider:SetValue(value, nocallback)
                            if typeof(value) == 'number' then
                                local newValue = clamp(
                                    self.increment
                                        * floor(value / self.increment),
                                    self.min,
                                    self.max
                                )
                                local size, pos =
                                    self.objects.slider.Size,
                                    self.objects.slider.Position
                                ...
                                self.objects.text.Text = string.format(
                                    '%.14g',
                                    newValue
                                ) .. tostring(
                                    self.suffix
                                ...
                                self.objects.text.ThemeColor = (
                                    self.min < 0 and newValue == 0
                                    or newValue == self.min
                                )
                                        and (self.risky and 'Risky Text' or 'Option Text 3')
                                    or (
                                        self.risky and 'Risky Text Enabled'
                                        or 'Option Text 1'
                                    )
                    ...
                    function toggle:AddList(data)
                            ...
                            utility:Connection(
                                ...
                                function()
                                    if list.open then
                                        ...
                                        window.dropdown.objects.background.Visible =
                                            true
                                        window.dropdown.objects.background.Parent =
                                            objs.holder
                                        window.dropdown:Refresh()
                                    end
                                end
                        ...
                        function list:Select(option, nocallback)
                            option = typeof(option) == 'table'
                                    and (self.multi == true and option or (#option == 0 and nil or option[1]))
                                or self.multi == true and { option }
                                or option
                            if option ~= nil then
                                self.selected = option
                                local text = typeof(option) == 'table'
                                        and (#option == 0 and 'none' or table.concat(
                                            option,
                                            ', '
                                        ))
                                    or tostring(option)
                        ...
                        tooltip(list)
                        list:Select(
                            (data.value or data.selected)
                                or (list.multi and 'none' or list.values[1]),
                            true
                        )
                        self:UpdateOptions()
                        return list
                ...
                function section:AddSlider(data)
                    ...
                    do
                        ...
                        utility:Connection(
                            slider.objects.plusDetector.MouseButton1Down,
                            function()
                                slider:SetValue(
                                    slider.value
                                        + (
                        ...
                        utility:Connection(
                            slider.objects.minusDetector.MouseButton1Down,
                            function()
                                slider:SetValue(
                                    slider.value
                                        - (
                            ...
                            function()
                                ...
                                then
                                    if slider.focused then
                                        ...
                                        objs.text.Text = slider.text
                                            .. ': '
                                            .. tostring(slider.value)
                                            .. tostring(slider.suffix)
                                            .. ' []'
                                        slider.focused = true
                                        inputNumber = ''
                                        c = utility:Connection(
                    ...
                    function slider:SetValue(value, nocallback)
                        if typeof(value) == 'number' then
                            local newValue = clamp(
                                self.increment * floor(value / self.increment),
                                self.min,
                                self.max
                            )
                            local size, pos =
                ...
                function section:AddButton(data)
                    ...
                    do
                        ...
                        utility:Connection(objs.holder.MouseLeave, function()
                            objs.border1.ThemeColor = 'Option Border 1'
                            objs.text.ThemeColor = self.risky and 'Risky Text'
                                or 'Option Text 3'
                        ...
                        utility:Connection(
                            objs.holder.MouseButton1Up,
                            function()
                                objs.text.ThemeColor = self.risky
                                        and 'Risky Text'
                                    or 'Option Text 3'
                                objs.background.ThemeColor = 'Option Background'
                                objs.background.ThemeColorOffset = 0
                    ...
                    function button:UpdateOptions() -- this so dumb XD
                        ...
                        local buttonSize = (1 / buttons) - 0.005
                        self.objects.background.Size =
                            newUDim2(buttonSize, -4, 0, 14)
                        for i, v in next, self.subbuttons do
                            v.objects.holder.Size =
                                newUDim2(buttonSize, 0, 1, 0)
                            v.objects.holder.Position =
                                newUDim2(i * buttonSize + 0.01, 0, 0, 0)
                        end
                    end
                ...
                function section:AddSeparator(data)
                    ...
                    for i, v in next, data do
                        if
                            ...
                            and (
                                separator[i] ~= nil
                                and typeof(separator[i]) == typeof(v)
                            )
                        then
                            separator[i] = v
                        end
                    ...
                    function separator:SetText(str)
                        if typeof(str) == 'string' then
                            ...
                            local xScale = (
                                1
                                - utility:ConvertNumberRange(
                                    self.objects.text.TextBounds.X,
                                    0,
                                    self.objects.holder.Object.Size.X,
                                    0,
                                    1
                                )
                            )
                            ...
                            self.objects.line1.Position =
                                newUDim2(0, 1, 0.5, -1)
                            self.objects.line2.Position = newUDim2(
                                1 - self.objects.line2.Size.X.Scale,
                                -1,
                                0.5,
                                -1
                            )
                ...
                function section:AddColor(data)
                    ...
                    do
                        ...
                        objs.text = utility:Draw('Text', {
                            ...
                            ThemeColor = color.risky and 'Risky Text Enabled'
                                or 'Option Text 3',
                            Size = 13,
                            Font = 2,
                            ZIndex = z + 1,
                        ...
                        utility:Connection(objs.holder.MouseLeave, function()
                            objs.border1.ThemeColor = color.state and 'Accent'
                                or 'Option Border 1'
                        end)
                ...
                function section:AddBox(data)
                    ...
                    do
                        ...
                        objs.text = utility:Draw('Text', {
                            Position = newUDim2(0, 2, 0, 2),
                            ThemeColor = box.risky and 'Risky Text Enabled'
                                or 'Option Text 2',
                            Size = 13,
                            Font = 2,
                            ZIndex = z + 1,
                    ...
                    function box:CaptureFocus(clear)
                        ...
                        c = utility:Connection(
                            ...
                            function(inp)
                                if
                                    inp.KeyCode == Enum.KeyCode.Return
                                    or inp.UserInputType
                                        == Enum.UserInputType.MouseButton1
                                then
                                ...
                                elseif inp.KeyCode == Enum.KeyCode.Escape then
                                    ...
                                    box:ReleaseFocus()
                                elseif
                                    inp.KeyCode == Enum.KeyCode.Backspace
                                then
                                    input = input:sub(1, -2)
                                    self.objects.inputText.Text = input
                                elseif
                                    ...
                                    or inp.KeyCode.Name == 'Space'
                                    or inp.KeyCode.Name == 'Minus'
                                    or inp.KeyCode.Name == 'Equals'
                                    or inp.KeyCode.Name == 'Backquote'
                                then
                                    local wlIdx = table.find(
                                        whitelistedBoxKeys,
                                        inp.KeyCode
                                    )
                                    local keyString = inp.KeyCode.Name
                                                == 'Space'
                                            and ' '
                                        or inp.KeyCode.Name == 'Minus' and '_'
                                        or inp.KeyCode.Name == 'Equals' and '+'
                                        or inp.KeyCode.Name == 'Backquote' and '~'
                                        or wlIdx ~= nil and tostring(wlIdx - 1)
                                        or inp.KeyCode.Name
                                    if
                                        not (
                                            inputservice:IsKeyDown(
                                            ...
                                            and not inputservice:IsKeyDown(
                ...
                function section:AddBind(data)
                    ...
                    do
                        ...
                        objs.text = utility:Draw('Text', {
                            Position = newUDim2(0, 2, 0, 2),
                            ThemeColor = bind.risky and 'Risky Text'
                                or 'Option Text 2',
                            Size = 13,
                            Font = 2,
                            ZIndex = z + 1,
                        ...
                        utility:Connection(objs.holder.MouseLeave, function()
                            objs.keyText.ThemeColor = bind.binding and 'Accent'
                                or 'Option Text 3'
                        end)
                    ...
                    function bind:SetKeyText(str)
                        str = tostring(str)
                        self.objects.keyText.Text = '[' .. str .. ']'
                        self.objects.keyText.Position = newUDim2(
                            1,
                            -self.objects.keyText.TextBounds.X,
                            0,
                            2
                        )
                    end
                    ...
                    utility:Connection(inputservice.InputBegan, function(inp)
                        ...
                        then
                            if bind.mode == 'toggle' then
                                ...
                                bind.callback(bind.state)
                                bind.indicatorValue:SetEnabled(
                                    bind.state and not bind.noindicator
                                )
                            elseif bind.mode == 'hold' then
                                if bind.flag then
                                    library.flags[bind.flag] = true
                ...
                function section:AddList(data)
                    ...
                    do
                        ...
                        objs.text = utility:Draw('Text', {
                            Position = newUDim2(0, 2, 0, 2),
                            ThemeColor = list.risky and 'Risky Text Enabled'
                                or 'Option Text 2',
                            Size = 13,
                            Font = 2,
                            ZIndex = z + 1,
                    ...
                    function list:Select(option, nocallback)
                        ...
                        if option ~= nil then
                            ...
                            if
                                label.TextBounds.X
                                > self.objects.background.Object.Size.X - 10
                            then
                                local split = text:split('')
                                for i = 1, #split do
                ...
                function section:AddText(data)
                    ...
                    do
                        ...
                        objs.text = utility:Draw('Text', {
                            Position = newUDim2(0, 2, 0, 2),
                            ThemeColor = text.risky and 'Risky Text Enabled'
                                or 'Option Text 2',
                            Size = 13,
                            Font = 2,
                            ZIndex = z + 1,
                    ...
                    function text:SetText(str)
                        if typeof(str) == 'string' then
                            ...
                            self.objects.holder.Size = newUDim2(
                                ...
                                0,
                                0,
                                self.objects.text.TextBounds.Y + 6
                            )
                            section:UpdateOptions()
                        end
                    end
            ...
            function tab:UpdateSections()
                ...
                for _, section in next, self.sections do
                    if
                        section.objects.background.Visible
                        ~= (section.enabled and tab.selected)
                    then
                        section.objects.background.Visible = section.enabled
                            and tab.selected
                        section:UpdateOptions()
                    end
                    ...
                    if section.enabled then
                        if section.side == 1 then
                            if last1 then
                                section.objects.background.Position = last1.objects.background.Position
                                    + newUDim2(
                                        0,
                                        0,
                                        0,
                                        last1.objects.background.Object.Size.Y
                                            + padding
                                    )
                            end
                            last1 = section
                        elseif section.side == 2 then
                            if last2 then
                                section.objects.background.Position = last2.objects.background.Position
                                    + newUDim2(
                                        0,
                                        0,
                                        0,
                                        last2.objects.background.Object.Size.Y
                                            + padding
                                    )
                            end
                            last2 = section
        ...
        function window:UpdateTabs()
            ...
            for i, v in next, self.tabs do
                ...
                objs.background.ThemeColor = v.selected
                        and 'Selected Tab Background'
                    or 'Unselected Tab Background'
                objs.background.Size = newUDim2(
                    0,
                    objs.text.TextBounds.X + 14,
                    1,
                    v.selected and 1 or 0
                )
                objs.background.Position = newUDim2(0, pos, 0, 0)

                objs.text.ThemeColor = v.selected and 'Selected Tab Text'
                    or 'Unselected Tab Text'
                objs.text.Position = newUDim2(0.5, 0, 0, 3)

                objs.topBorder.ThemeColor = v.selected and 'Accent'
                    or 'Unselected Tab Background'
        ...
        function self.watermark:Update()
            ...
            if library.flags.watermark_enabled then
                ...
                date[2] = date[2]
                    .. (
                        daySuffix == 1 and 'st'
                        or daySuffix == 2 and 'nd'
                        or daySuffix == 3 and 'rd'
                        or 'th'
                    )
                ...
                self.objects.background.Size =
                    newUDim2(0, self.objects.text.TextBounds.X + 10, 0, 17)
                ...
                self.position = (
                    self.lock == 'Top Right'
                        and newUDim2(0, screensize.X - size.X - 15, 0, 15)
                    or self.lock == 'Top Left' and newUDim2(0, 15, 0, 15)
                    or self.lock == 'Bottom Right' and newUDim2(
                        0,
                        screensize.X - size.X - 15,
                        0,
                        screensize.Y - size.Y - 15
                    )
                    or self.lock == 'Bottom Left' and newUDim2(
                        0,
                        15,
                        0,
                        screensize.Y - size.Y - 15
                    )
                    or self.lock == 'Top' and newUDim2(
                        0,
    ...
    self.keyIndicator = self.NewIndicator({
        title = 'Keybinds',
        pos = newUDim2(0, 15, 0, 325),
        enabled = true,
    })

    self.targetIndicator = self.NewIndicator({
        title = 'Target Info',
        pos = newUDim2(0, 15, 0, 350),
        enabled = false,
    })
    self.targetName =
        self.targetIndicator:AddValue({ key = 'Name     :', value = 'nil' })
    self.targetDisplay =
        self.targetIndicator:AddValue({ key = 'DName    :', value = 'nil' })
    self.targetHealth =
        self.targetIndicator:AddValue({ key = 'Health   :', value = '0' })
    self.targetDistance =
        self.targetIndicator:AddValue({ key = 'Distance :', value = '0m' })
    self.targetTool =
        self.targetIndicator:AddValue({ key = 'Weapon   :', value = 'nil' })
...
function library:CreateSettingsTab(menu)
    ...
    local function refreshConfigs()
        ...
        for _, v in
            next,
            listfiles(self.cheatname .. '/' .. self.gamename .. '/configs')
        do
            local ext = '.' .. v:split('.')[#v:split('.')]
            if ext == self.fileext then
                library.options.selectedconfig:AddValue(
                    v:split('\\')[#v:split('\\')]:sub(1, -#ext - 1)
                )
            end
        end
    end
    ...
    mainSection:AddButton({
        ...
        callback = function()
            ...
            if res.Success then
                library:SendNotification(
                    library.cheatname .. ' | Joined Discord',
                    3
                )
            end
        end,
    ...
    mainSection:AddSlider({
        ...
        value = 0.5,
        callback = function()
            library.keyIndicator:SetPosition(
                newUDim2(
                    library.flags.keybind_indicator_x / 100,
                    0,
                    library.flags.keybind_indicator_y / 100,
                    0
                )
            )
        end,
